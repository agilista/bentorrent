 ...
 */
public final class Announce implements Runnable {

	protected static final SlfLogger logger = SlfLoggerFactory.getLogger(Announce.class);
	...
	public Announce(SharedTorrent torrent, Peer peer) {
		...
		for (List<URI> tier : torrent.getAnnounceList()) {
			...
			for (URI tracker : tier) {
				try {
					...
					logger.warn("Will not announce on {0}: {1}!",
						tracker,
						e.getMessage() != null
							? e.getMessage()
		...
		logger.info("Initialized announce sub-system with {0} trackers on {1}.",
	...
	public void setInterval(int interval) {
		...
		logger.info("Setting announce interval to {0}s per tracker request.",
			interval);
		...
		while (!this.stop) {
			try {
				...
				try {
					...
					logger.error("Unable to move to the next tracker client: {0}", e.getMessage());
				}
			}
	...
	private void promoteCurrentTrackerClient() throws AnnounceException {
		logger.trace("Promoting current tracker client for {0} " +
			"(tier {1}, position {2} -> 0).",
			new Object[] {
	...
	private void moveToNextTrackerClient() throws AnnounceException {
		...
		if (tier != this.currentTier ||
			...
			logger.debug("Switched to tracker client for {0} " +
				"(tier {1}, position {2}).",
				new Object[] {
					this.getCurrentTrackerClient().getTrackerURI(),
					this.currentTier,
