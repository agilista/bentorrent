...
import uk.co.itstherules.external.ApacheByteArrayOutputStream;
...
public final class HTTPTrackerClient extends TrackerClient {
	...
	private static final SlfLogger logger = SlfLoggerFactory.getLogger(HTTPTrackerClient.class);
	...
	public void announce(AnnounceRequestMessage.RequestEvent event, boolean inhibitEvents) throws AnnounceException {
		logger.info("Announcing{0} to tracker with {1}U/{2}D/{3}L bytes...",
                formatAnnounceEvent(event), torrent.getUploaded(), torrent.getDownloaded(), torrent.getLeft());

		URL target = null;
		try {
			HTTPAnnounceRequestMessage request = buildAnnounceRequest(event);
			target = request.buildAnnounceURL(tracker.toURL());
		...
		InputStream in = null;
		try {
			conn = HttpURLConnection.class.cast(target.openConnection());
		...
		try {
			ApacheByteArrayOutputStream baos = new ApacheByteArrayOutputStream();
			baos.write(in);
	 ...
	 */
	private HTTPAnnounceRequestMessage buildAnnounceRequest(AnnounceRequestMessage.RequestEvent event) throws IOException, MessageValidationException {
		// Build announce request message
		return HTTPAnnounceRequestMessage.craft(torrent.getInfoHash(), peer.getPeerId().array(), peer.getPort(),
                torrent.getUploaded(), torrent.getDownloaded(), torrent.getLeft(), true, false, event, peer.getIp(),
