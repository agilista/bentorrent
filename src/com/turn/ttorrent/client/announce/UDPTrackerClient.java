...
import static com.turn.ttorrent.common.protocol.TrackerMessage.AnnounceRequestMessage.RequestEvent.STOPPED;
 ...
 * @author mpetazzoni
 */
public final class UDPTrackerClient extends TrackerClient {

	protected static final SlfLogger logger = SlfLoggerFactory.getLogger(UDPTrackerClient.class);
	...
	public void announce(AnnounceRequestMessage.RequestEvent event,
		...
		int maxAttempts = STOPPED.equals(event) ? UDP_MAX_TRIES_ON_STOPPED : UDP_MAX_TRIES;
		int attempts = -1;
	...
	private void send(ByteBuffer data) {
		try {
			...
			logger.warn("Error sending datagram packet to tracker at {0}: {1}.",
	...
	private ByteBuffer recv(int attempt)
		...
		logger.trace("Setting receive timeout to {0}s for attempt {1}...",
			timeout, attempt);
