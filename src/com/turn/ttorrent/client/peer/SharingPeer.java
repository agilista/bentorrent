...
public class SharingPeer extends Peer implements MessageListener {
	...
	private static final SlfLogger logger =
		SlfLoggerFactory.getLogger(SharingPeer.class);
	...
	public void interesting() {
		if (!this.interesting) {
			logger.trace("Telling {0} we're interested.", this);
	...
	public void notInteresting() {
		if (this.interesting) {
			logger.trace("Telling {0} we're no longer interested.", this);
	...
	public void send(PeerMessage message) throws IllegalStateException {
		if (this.isConnected()) {
			...
			logger.warn("Attempting to send a message to non-connected peer {0}!", this);
		}
	}
	...
	public synchronized void downloadPiece(Piece piece)
		...
		if (this.isDownloading()) {
			...
			logger.warn("What's going on? {0}", up.getMessage(), up);
			throw up; // ah ah.
		}
	...
	public synchronized void handleMessage(PeerMessage msg) {
		switch (msg.getType()) {
			...
			case UNCHOKE:
				this.choked = false;
				logger.trace("Peer {0} is now accepting requests.", this);
				this.firePeerReady();
				break;
			case INTERESTED:
			...
			case HAVE:
				...
				synchronized (this.availablePieces) {
					...
					logger.trace("Peer {0} now has {1} [{2}/{3}].",
						new Object[] {
							this,
							havePiece,
			...
			case BITFIELD:
				...
				synchronized (this.availablePieces) {
					...
					logger.trace("Recorded bitfield from {0} with {1} " +
						"pieces(s) [{2}/{3}].",
						new Object[] {
							this,
			...
			case REQUEST:
				...
				if (this.isChoking() || !rp.isValid()) {
					logger.warn("Peer {0} violated protocol, " +
				...
				if (request.getLength() >
					...
					logger.warn("Peer {0} requested a block too big, " +
						"terminating exchange.", this);
					this.unbind(true);
					break;
				}
