...
class PeerExchange {
	...
	private static final SlfLogger logger =
		SlfLoggerFactory.getLogger(PeerExchange.class);
	...
	public PeerExchange(SharingPeer peer, SharedTorrent torrent,
		...
		logger.debug("Started peer exchange with {0} for {1}.",
	...
	public void close() {
		...
		logger.debug("Peer exchange with {0} closed.", this.peer);
	}
			...
			try {
				while (!stop) {
					...
					try {
						...
						logger.trace("Received {0} from {1}", message, peer);
						...
						logger.warn("{0}", pe.getMessage());
					}
				}
			} catch (IOException ioe) {
				logger.debug("Could not read message from {0}: {1}",
					peer,
					ioe.getMessage() != null
						? ioe.getMessage()
			...
			try {
				...
				while (!stop || (stop && sendQueue.size() > 0)) {
					try {
						...
						logger.trace("Sending {0} to {1}", message, peer);
				...
				logger.debug("Could not send message to {0}: {1}",
					peer,
					ioe.getMessage() != null
						? ioe.getMessage()
						: ioe.getClass().getName());
