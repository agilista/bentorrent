...
import static com.turn.ttorrent.client.Client.ClientState.*;
import static com.turn.ttorrent.common.protocol.TrackerMessage.AnnounceRequestMessage.RequestEvent.COMPLETED;
...
public class Client extends Observable implements Runnable, AnnounceResponseListener, IncomingConnectionListener,
    ...
    private static final SlfLogger logger = SlfLoggerFactory.getLogger(Client.class);

    /**
     * Peers unchoking frequency, in seconds. Current BitTorrent specification recommends 10 seconds to avoid choking
     * fibrilation.
     */
    ...
    public enum ClientState {WAITING, VALIDATING, SHARING, SEEDING, ERROR, DONE}
    ...
    private Map<String, SharingPeer> peers;
    private Map<String, SharingPeer> connected;
     ...
     */
    public Client(InetAddress address, SharedTorrent torrent) throws IOException {
        ...
        service = new ConnectionHandler(this.torrent, id, address);
        service.register(this);

        self = new Peer(service.getSocketAddress().getAddress().getHostAddress(),
                (short) service.getSocketAddress().getPort(), ByteBuffer.wrap(id.getBytes(Torrent.BYTE_ENCODING)));

        // Initialize the announce request thread, and register ourselves to it as well.
        announce = new Announce(this.torrent, self);
        announce.register(this);

        logger.info("BitTorrent client [{0}] for {1} started and listening at {2}:{3}...",
                self.getShortHexPeerId(), this.torrent.getName(), self.getIp(), self.getPort()
        );
    ...
    public void setMaxDownloadRate(double rate) {
        torrent.setMaxDownloadRate(rate);
    }
    ...
    public synchronized void share(int seed) {
        ...
        if (thread == null || !thread.isAlive()) {
            ...
            thread.setName("bt-client(" + self.getShortHexPeerId() + ")");
            thread.start();
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            logger.warn("Error while initializing torrent data: {0}!", ioe.getMessage(), ioe);
        } catch (InterruptedException ie) {
            ...
            if (!torrent.isInitialized()) {
                try {
                    ...
                    logger.warn("Error while releasing bound channel: {0}!", ioe.getMessage(), ioe);
                }
        ...
        while (!stop) {
            optimisticIterations = (optimisticIterations == 0 ? Client.OPTIMISTIC_UNCHOKE_ITERATIONS : optimisticIterations - 1);
            rateComputationIterations = (rateComputationIterations == 0 ? Client.RATE_COMPUTATION_ITERATIONS : rateComputationIterations - 1);
    ...
    public synchronized void info() {
        ...
        logger.info("{0} {1}/{2} pieces ({3}%) [{4}/{5}] with {6}/{7} peers at {8}/{9} kB/s.",
                getState().name(),
        ...
        for (SharingPeer peer : connected.values()) {
            ...
            logger.debug("  | {0} {1}", peer, (piece != null ? "(downloading " + piece + ")" : ""));
        }
    }
    ...
    private SharingPeer getOrCreatePeer(Peer search) {
        ...
        synchronized (peers) {
            logger.trace("Searching for {0}...", search);
            if (search.hasPeerId()) {
                peer = peers.get(search.getHexPeerId());
                if (peer != null) {
                    logger.trace("Found peer (by peer ID): {0}.", peer);
                    peers.put(peer.getHostIdentifier(), peer);
                    peers.put(search.getHostIdentifier(), peer);
                    return peer;
                }
            }

            peer = peers.get(search.getHostIdentifier());
            if (peer != null) {
                if (search.hasPeerId()) {
                    logger.trace("Recording peer ID {0} for {1}.",
                            search.getHexPeerId(), peer);
                    peer.setPeerId(search.getPeerId());
                    peers.put(search.getHexPeerId(), peer);
                }

                logger.debug("Found peer (by host ID): {0}.", peer);
                return peer;
            }
            peer = new SharingPeer(search.getIp(), search.getPort(), search.getPeerId(), torrent);
            logger.trace("Created new peer: {0}.", peer);
            peers.put(peer.getHostIdentifier(), peer);
            if (peer.hasPeerId()) {
                peers.put(peer.getHexPeerId(), peer);
            }
            return peer;
        }
    }
    ...
    private Comparator<SharingPeer> getPeerRateComparator() {
        if (SHARING.equals(state)) {
            return new SharingPeer.DLRateComparator();
        } else if (SEEDING.equals(state)) {
            return new SharingPeer.ULRateComparator();
        } else {
            throw new IllegalStateException("Client is neither sharing nor seeding, we shouldn't be comparing peers at " +
                    "this point.");
        }
    }
    ...
    private synchronized void unchokePeers(boolean optimistic) {
        ...
        // not connected to.
        TreeSet<SharingPeer> bound = new TreeSet<>(getPeerRateComparator());
        ...
        if (bound.size() == 0) {
            ...
            logger.trace("Running unchokePeers() on {0} connected peers.", bound.size());
        }
        ...
        // optimistic unchoke.
        if (choked.size() > 0) {
            SharingPeer randomPeer = choked.toArray(new SharingPeer[0])[random.nextInt(choked.size())];

            for (SharingPeer peer : choked) {
                if (optimistic && peer == randomPeer) {
                    logger.debug("Optimistic unchoke of {0}.", peer);
                    continue;
                }
    ...
    public void handleDiscoveredPeers(List<Peer> peers) {
        ...
        logger.info("Got {0} peer(s) in tracker response.", peers.size());
    ...
    public void handleNewPeerConnection(SocketChannel channel, byte[] peerId) {
        Peer search = new Peer(channel.socket().getInetAddress().getHostAddress(), channel.socket().getPort(),
                (peerId != null ? ByteBuffer.wrap(peerId) : null));

        logger.info("Handling new peer connection with {0}...", search);
        SharingPeer peer = getOrCreatePeer(search);

        try {
            synchronized (peer) {
                if (peer.isConnected()) {
                    logger.info("Already connected with {0}, closing link.", peer);
                    channel.close();
                    return;
                }
                peer.register(this);
            ...
            connected.put(peer.getHexPeerId(), peer);
            peer.register(torrent);
            logger.debug("New peer connection with {0} [{1}/{2}].", peer, this.connected.size(), this.peers.size());
        } catch (Exception e) {
            connected.remove(peer.getHexPeerId());
            logger.warn("Could not handle new peer connection with {0}: {1}", peer, e.getMessage());
        }
    }
    ...
    public void handleFailedConnection(SharingPeer peer, Throwable cause) {
        logger.warn("Could not connect to {0}: {1}.", peer, cause.getMessage());
    ...
    @Override
    public void handlePieceAvailability(SharingPeer peer, Piece piece) { /* Do nothing */ }
    ...
    public void handlePieceCompleted(SharingPeer peer, Piece piece) throws IOException {
        synchronized (torrent) {
            if (piece.isValid()) {
                ...
                logger.debug("Completed download of {0} from {1}. We now have {2}/{3} pieces",
                        piece, peer, torrent.getCompletedPieces().cardinality(), torrent.getPieceCount()
                );

                // Send a HAVE message to all connected peers
                PeerMessage have = PeerMessage.HaveMessage.craft(piece.getIndex());
                for (SharingPeer remote : connected.values()) {
                    remote.send(have);
                }
                ...
                logger.warn("Downloaded piece#{0} from {1} was not valid ;-(", piece.getIndex(), peer);
            }
            ...
            if (torrent.isComplete()) {
                ...
                for (SharingPeer remote : connected.values()) {
                    if (remote.isDownloading()) {
                        ...
                        logger.info("Cancelled {0} remaining pending requests on {1}.", requests, remote);
                    }
                }
                torrent.finish();
                try {
                    announce.getCurrentTrackerClient().announce(COMPLETED, true);
                } catch (AnnounceException ae) {
                    logger.warn("Error announcing completion event to tracker: {0}", ae.getMessage());
                }
    ...
    public void handlePeerDisconnected(SharingPeer peer) {
        if (connected.remove(peer.hasPeerId() ? peer.getHexPeerId() : peer.getHostIdentifier()) != null) {
            logger.debug("Peer {0} disconnected, [{1}/{2}].", peer, connected.size(), peers.size());
        }
        peer.reset();
    }
    ...
    public void handleIOException(SharingPeer peer, IOException ioe) {
        logger.warn("I/O error while exchanging data with {0}, closing connection with it!", peer, ioe.getMessage());
        peer.unbind(true);
    }
    ...
    private synchronized void seed() {
        ...
        if (SEEDING.equals(getState())) {
            return;
        }

        logger.info("Download of {0} pieces completed.", torrent.getPieceCount());
        ...
        logger.info("Seeding for {0} seconds...", seed);
        Timer timer = new Timer();
        timer.schedule(new ClientShutdown(this, timer), seed * 1000);
    }
