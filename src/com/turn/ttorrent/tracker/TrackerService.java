...
public class TrackerService implements Container {
	...
	private static final SlfLogger logger =
		SlfLoggerFactory.getLogger(TrackerService.class);
	...
	public void handle(Request request, Response response) {
		...
		try {
			...
			logger.warn("Error while writing response: {0}!", ioe.getMessage());
		} finally {
			if (body != null) {
				try {
					body.close();
	...
	private void process(Request request, Response response,
		...
		if (torrent == null) {
			logger.warn("Requested torrent hash was: {0}",
				announceRequest.getHexInfoHash());
			this.serveError(response, body, Status.BAD_REQUEST,
	...
	private void serveError(Response response, OutputStream body,
		...
		logger.warn("Could not process announce request ({0}) !",
			error.getReason());
